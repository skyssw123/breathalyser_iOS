...
@objc protocol MqttManagerDelegate : class {
...
@objc class MqttManager : NSObject
{
    ...
    func setStatus(value: ConnectionStatus )
    {
        status = value;
    }
    ...
    func connect(host: String, port: Int, username: String?, password: String?, cleanSession: Bool) {
        ...
        mqttConfig.onConnectCallback = { [ weak delegate = self.delegate,  unowned self /*, weak mqttClient = self.mqttClient*/ ] returnCode  in
            printLog("" as AnyObject, funcName: (#function), logString: "MQTT connectedCallback \(returnCode.description)")
